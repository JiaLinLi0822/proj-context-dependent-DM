...
def optimizefun(pars, actions, muvalue, totaldays, trial_num):
    ...
    k, b, sigma, beta, alpha = pars[0], pars[1], pars[2], pars[3], pars[4]
...
def fit_subject(file, folder, N_iter, output_dir):
    ...
    for _ in range(N_iter):
        pars0 = np.random.uniform(1e-4, 1, 5)
        pars0[1] = np.random.uniform(1e0, 5e0)

        lb = np.array([1e-5, 1e0, 1e-5, 1e-5, 1e-5])
        ub = np.array([5e0, 5e0, 2e0, 1e2, 2e0])

        pars = constrNMPy.constrNM(optimizefun, pars0, lb, ub, args=(actions, muvalue, totaldays, trial_num), 
    ...
    fitting = pd.DataFrame(parOpt, columns=['k', 'b', 'sigma', 'beta', 'alpha'])
    fitting = pd.merge(fitting, pd.DataFrame(fopt), left_index=True, right_index=True)
    fitting.columns = ['k', 'b', 'sigma', 'beta', 'alpha', 'loglik', 'nIter', 'FunEvals', 'warnflag']
...
def modelfitting(model, N_iter):
    ...
    for folder in folder_path:
        ...
        if model == 1:
            output_path = "C:/Users/ljl22/Desktop/毕业设计/models/new/PropVrisk/"
            ...
            res = pd.DataFrame(results, columns=['file', 'k', 'b', 'sigma', 'beta', 'alpha', 'loglik', 'nIter', 'FunEvals', 'warnflag'])
            res.to_csv(output_dir + 'opt.csv', index=False)

    return

if __name__ == '__main__':
    modelfitting(model=1, N_iter=1)
