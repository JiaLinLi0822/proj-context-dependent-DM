...
var correctanswerdisplay=["-2.9","-2.4"];
		...
		if(questionnum<=2){
			if(correct_or_wrong===1){
				...
				if(questionnum===1){
					...
					$('#choice2').text('得到-5.0到-1.0之间的评分；每一个评分都将是等可能的。');
					$('#choice3').text('只能得到评分在-2.9分以下的评分。');
					$('#choice4').text('以上说法均不正确，我只能知道我能够得到-5.0到-1.0之间的评分。');
					$('#quiz2choice4').show();
				}
				else{
				...
				if(trytime===1){
					...
					if(questionnum===1){
						...
						$('#choice3').text('只能得到评分在-2.4分以下的评分。');
					}
					trytime++;
				}
				else{
